...
#Hàm update
def update_sv(connection,metadata, engine,hoten,idstudent):
    sinhvien = db.Table('sinhvien', metadata, autoload=True, autoload_with=engine)
    query_insert = db.update(sinhvien).values(hoten=hoten).where(sinhvien.columns.idstudent == idstudent)
    ResultProxy = connection.execute(query_insert)
    return ResultProxy

#Hàm delete
def delete_sv(connection,metadata, engine,idstudent):
    sinhvien = db.Table('sinhvien', metadata, autoload=True, autoload_with=engine)
    query_delete = db.delete(sinhvien).where(sinhvien.columns.idstudent == idstudent)
