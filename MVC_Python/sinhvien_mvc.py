...
def start():
    model = sinhvien_model.SinhVienModel("localhost", "root", "llăêđ̉âđăê", "qlsv")
    ...
    while item in ["1", "2", "3", "4"]:
        if item == "1":
            ...
            hoten = input("Nhập họ tên: ")
            idstudent = input("Nhập Id:")
            controller.update_sinhvien(hoten,idstudent)
        elif item == "4":
            idstudent = input("Nhập Id: ")
            controller.delete_sinhvien(idstudent)
